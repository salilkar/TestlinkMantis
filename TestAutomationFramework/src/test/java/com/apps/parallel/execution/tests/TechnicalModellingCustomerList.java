...
public class TechnicalModellingCustomerList extends BaseClass{
		...
		try {
			...
			try {
				...
				reports.writeIntoFile(driver, testCaseName, "Validate Customer list ", "Login with valid credentials",
						"Customer list is displayed", reports.pass, "", comlib.getCurrentTime());
			} catch (Error e) {
				reports.writeIntoFile(driver, testCaseName, "Validate Customer List", "Login with valid credentials",
						"Customer list is not displayed", reports.fail, e.getMessage(), comlib.getCurrentTime());
			}
